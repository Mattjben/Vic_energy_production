...
no significant lag in acf plot and 1 significant lag at lag 13 aka lag 1 - so we will try a SIMRA(1,0) to see if we can get rid of the wave alltogether

```{r}
# Model 2 SAMRA(1,0) with first seasonal differencing
m2.ts<-Plot_Model(diff.Data_ts_agg,1,"X",0,0,0,1,1,0)
```
significant lag at pacf 1 still so try a p value of 2 

```{r}
# Model 3 SAMRA(2,0) with first seasonal differencing
m3.ts<-Plot_Model(diff.Data_ts_agg,1,"X",0,0,0,2,1,0)
```
...
Arima(diff.Data_ts_agg,order=c(1,0,1),seasonal=list(order=c(2,1,0), period=12))
```
